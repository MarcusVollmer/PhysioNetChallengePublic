function mvqrs_debugmode_gui2(debug_file)

% Initialize and hide the GUI as it is being constructed.
f=figure('Visible','off','Position',[0,0,700,400],'Units','normalized','PaperSize',[20,15]);%,'ResizeFcn',@setmarkersize
...
function popup_Callback(source,~)
    ...
    s = source.String;
    x = str2num(s(val,:)); 
    xlim([round(x/Fs)-3 round(x/Fs)+3])   
end
...
%         gap = .1/(length(ValidSignals)-1);
    ...
    for i=1:length(ValidSignals)
        ax((i-1)*2+1) = subplot('Position',[.075,.1+(i-1)*(.7/length(ValidSignals)+gap),...
            .4,.7/length(ValidSignals)]);
        ...
        set(gca,'YTick',[],'XTick',refAnn/(factor*Fs),'XTickLabel',[])
        grid on
        if i==length(ValidSignals)
            [~, deepestFolder, ~] = fileparts(pwd);
            title([deepestFolder '/' recordName],'Color',c1)
        end
        hold off

        
        ax((i-1)*2+2) = subplot('Position',[.525,.1+(i-1)*(.7/length(ValidSignals)+gap),...
            .4,.7/length(ValidSignals)]);
        ...
        set(gca,'XColor',c1,'YColor',c1,'Color',c2+(.3*c1))
        if i==length(ValidSignals)
            title('filtered time series','Color',c1)
        end
        if i==1
            xlabel(gca,'Beat number')
            set(gca,'XTick',refAnn/(factor*Fs),'XTickLabel',1:length(refAnn),'XTickLabelRotation',90,'XGrid','on','YTickLabel',[])
        else
            set(gca,'XTick',refAnn/(factor*Fs),'XTickLabel',[],'XGrid','on','YTickLabel',[])
        end    
%         set(gca,'YTickLabel',[])
    end
